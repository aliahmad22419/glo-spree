        ...
        klass.class_eval do
          ...
          def self.define_state_machine!
            ...
            state_machine :state, initial: :cart, use_transactions: false, action: :save_state do
              ...
              event :confirm do
                transition to: :confirm, from: [:payment, :delivery]
              end
              ...
              before_transition to: :complete, do: :ensure_non_zero_cart
              before_transition to: :complete, do: :ensure_payments_via_gift_card_are_captured
              after_transition to: :complete, do: :save_line_item_exchange_rates
              after_transition to: :complete, do: :increase_client_order_id
              after_transition to: :complete, do: :capture_stripe_payment
              after_transition to: :complete, do: :create_test_order_tags

              #update all single page orders addresses same as billing address
              after_transition to: :complete do |order|
                order.ship_address.update(order.bill_address.attributes.except("id")) if order.store.preferred_single_page
              end
              ...
              if states[:payment]
                before_transition to: :complete do |order|
                  if order.payment_required? && order.payments.valid.empty?
                    ...
                    order.process_payments! unless order.payment_through_adyen? or order.payment_method_type&.include? "Eghl"
                    order.send(:paid_completely)
                  end
                end
              ...
              after_transition to: :complete, do: :generate_gift_cards
              # after_transition to: :complete, do: :generate_hawk_cards
              after_transition to: :complete, do: :generate_ts_cards
              after_transition to: :resumed, do: :after_resume
              after_transition to: :canceled, do: :after_cancel
              after_transition to: :complete, do: :send_data_to_sqs
              after_transition to: :complete, do: :delete_cache_after_checkout
              after_transition to: :complete, do: :log_card_email_change
              # after_transition to: :complete, do: :set_completion_config
          ...
          def log_card_email_change
            self.line_items.where(delivery_mode: DIGITAL_TYPES).each do |line_item|
              line_item.email_changes.create(previous_email: line_item.receipient_email, next_email: line_item.receipient_email, note: "system email change") unless line_item.email_changes.present?
            end
          end

          def set_completion_config
            self.update_preferences({
                                      single_page: store.preferred_single_page,
                                      decimal_points: store.decimal_points,
                                      currency_formatter: store.currency_formatter,
                                      stripe_connected_account: store.stripe_connected_account
                                    })
          end

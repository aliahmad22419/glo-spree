# duplicate -- no need but without it not merging promotions properly,
# may be some other spree extention is overriding this file
module Spree
  class OrderMerger
    attr_accessor :order
    ...
    def merge!(other_order, user = nil)
      ...
      order.promotions += other_order.promotions
    ...
    def find_matching_line_item(other_order_line_item)
      order.line_items.detect do |my_li|
        customization_options = other_order_line_item.line_item_customizations.map{ |customization| customization.serializable_hash.transform_keys(&:to_sym) }
        my_li.variant == other_order_line_item.variant &&
          Spree::Dependencies.cart_compare_line_items_service.constantize.new.call(order: order,
                                                                                   line_item: my_li,
                                                                                   options: other_order_line_item.serializable_hash.transform_keys(&:to_sym).merge({customization_options: customization_options})).value
      end
    end

...
prawn_document(force_download: true) do |pdf|
  ...
  if @doc.card_type == :monetary
    pdf.grid([1,0], [6,4]).bounding_box do
        render 'spree/printables/shared/giftcard/monetary', pdf: pdf, giftcard: @doc.printable
    end
  elsif @doc.card_type == :experiences
    pdf.grid([1,0], [6,4]).bounding_box do
       render 'spree/printables/shared/giftcard/experience', pdf: pdf, giftcard: @doc.printable
    end
  end
